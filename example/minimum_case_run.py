...
if __name__ == "__main__":
    target_res_mapping = {
        ...
        500: 0.0045,
        1000: 0.009,
    }  # map target resolution in meters to degrees

    # ---define some variables for the path look-ups (optional)
    tmp_dir = (
        "tmp"  # path of a directory for temporary results saving during prediction
    )
    ...
    target_resolution = (
        100  # target resolution for building height and footprint mapping
    )
    target_extent = [
        -4.4786000,
        55.6759000,
        -3.8828000,
        56.0197000,
    ]  # target extent for building height and footprint mapping
    ...
    input_img = {
        "50pt": {  # use annual medians as aggregation operation for one year data
            ...
            ),  # path of input Sentinel-1 image
            s2_key: os.path.join(
                case_prefix,
                "infer_test_Glasgow",
                "raw_data",
                "Glasgow_2020_sentinel_2.tif",
            ),  # path of input Sentinel-2 image
        }
    }
    ...
    aux_data = {
        "DEM": {
            "path": os.path.join(
                case_prefix, "infer_test_Glasgow", "raw_data", "Glasgow_srtm.tif"
            ),  # path of input SRTM data
    ...
    pretrained_model_path = os.path.join(
        "check_pt_{0}_100m_MTL".format(backbone), "experiment_1", "checkpoint.pth.tar"
    )  # path of files of pretrained models
    ...
    )  # check whether CUDA can be used for prediction
    ...
    output_footprint_file = (
        "_".join([output_prefix, "footprint", backbone + "_MTL"]) + ".tif"
    )
    output_footprint_path = os.path.join(
        output_dir, output_footprint_file
    )  # path of output building footprint files
    output_height_file = "_".join([output_prefix, "height", backbone + "_MTL"]) + ".tif"
    output_height_path = os.path.join(
        output_dir, output_height_file
    )  # path of output building height files
    ...
    pred_height_from_tiff_DL_patch_MTL(
        ...
        predictor=model,
        trained_record=pretrained_model_path,
        resolution=target_res_mapping[target_resolution],
        s1_key=s1_key,
        s2_key=s2_key,
        aux_feat_info=aux_data,
        crossed=False,
        base_dir=tmp_dir,
        padding=padding,
