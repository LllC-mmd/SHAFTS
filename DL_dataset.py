...
def HDF5_to_LMDB(h5_dataset_path, lmdb_dataset_path, target_list, aggregation_list, s1_prefix="sentinel_1", s2_prefix="sentinel_2", scale=0.0001, max_size=1099511627776):
...
class PatchDatasetFromLMDB(Dataset):
    def __init__(self, lmdb_dataset_path, aggregation_list, num_band=6, scale=0.0001, cached=True, mode="train", target_id_shift=1, log_scale=False):
...
class PatchDatasetFromLMDB_MTL(Dataset):
    def __init__(self, lmdb_dataset_path, aggregation_list, num_band=6, scale=0.0001, cached=True, mode="train",
                 footprint_id_shift=1, height_id_shift=2, log_scale=False):
        self.db_path = lmdb_dataset_path
        self.num_aggregation = len(aggregation_list)
        self.num_band = num_band
        self.scale = scale
...
class PatchDatasetFromHDF5(Dataset):
    def __init__(self, h5_dataset_path, target_variable, aggregation_list, s1_prefix="sentinel_1", s2_prefix="sentinel_2", scale=0.0001, cached=True, mode="train", log_scale=False):
        self.scale = scale
        self.db = None
...
class PatchDatasetFromHDF5_infer(Dataset):
    def __init__(self, h5_dataset_path, aggregation_list, s1_prefix="sentinel_1", s2_prefix="sentinel_2", scale=0.0001, cached=True):
        self.scale = scale
        self.db = None
        self.ds_path = h5_dataset_path
        ...
        self.group_namelist = sorted(self.feature_info.keys())
        self.aggregation_list = aggregation_list
        self.channel_ref = {}
        band_ref = {"VV": "B1", "VH": "B2", "R": "B1", "G": "B2", "B": "B3", "NIR": "B4"}
        for aggregation_ops in aggregation_list:
        ...
        test_key = self.channel_ref[self.aggregation_list[0]]["S1_VV"]
        self.num_sample_acc = np.cumsum([self.feature_info[g][test_key]["shape"][0] for g in self.group_namelist])
        self.num_sample_acc = np.concatenate([[0], self.num_sample_acc], axis=0)
    ...
    def __getitem__(self, index):
        ...
        for aggregation_ops in self.channel_ref.keys():
            ...
            patch = np.concatenate([s1_vv_coef_patch, s1_vh_coef_patch,
...
def load_data_hdf5(h5_dataset_path, batch_size, num_workers, target_variable, aggregation_list, mode, cached=True, log_scale=False):
    if mode == "train":
        patch_dataset = PatchDatasetFromHDF5(h5_dataset_path, target_variable, aggregation_list, mode="train", cached=cached, log_scale=log_scale)
        data_loader = DataLoader(patch_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    elif mode == "valid":
        patch_dataset = PatchDatasetFromHDF5(h5_dataset_path, target_variable, aggregation_list, mode="valid", cached=cached, log_scale=log_scale)
        data_loader = DataLoader(patch_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    elif mode == "infer":
        patch_dataset = PatchDatasetFromHDF5_infer(h5_dataset_path, aggregation_list, cached=cached)
        data_loader = DataLoader(patch_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    else:
        raise NotImplementedError
...
def load_data_lmdb(lmdb_dataset_path, batch_size, num_workers, aggregation_list, mode, target_id_shift=1, cached=True, log_scale=False):
    if mode == "train":
        patch_dataset = PatchDatasetFromLMDB(lmdb_dataset_path, aggregation_list, mode="train", target_id_shift=target_id_shift, cached=cached, log_scale=log_scale)
        data_loader = DataLoader(patch_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    elif mode == "valid":
        patch_dataset = PatchDatasetFromLMDB(lmdb_dataset_path, aggregation_list, mode="valid", target_id_shift=target_id_shift, cached=cached, log_scale=log_scale)
        data_loader = DataLoader(patch_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    else:
        raise NotImplementedError
...
def load_data_lmdb_MTL(lmdb_dataset_path, batch_size, num_workers, aggregation_list, mode, cached=True, log_scale=False):
    if mode == "train":
        patch_dataset = PatchDatasetFromLMDB_MTL(lmdb_dataset_path, aggregation_list, mode="train", cached=cached,
                                                 footprint_id_shift=1, height_id_shift=2, log_scale=log_scale)
        data_loader = DataLoader(patch_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    elif mode == "valid":
...
if __name__ == "__main__":
    ...
    for res in ["100m", "250m", "500m", "1000m"]:
        ...
        HDF5_to_LMDB(h5_dataset_path=os.path.join(path_prefix, "patch_data_50pt_s{0}_{1}_train.h5".format(scale, res)),
                     ...
                     s1_prefix="sentinel_1",
                     s2_prefix="sentinel_2", scale=1.0, max_size=1099511627776)
